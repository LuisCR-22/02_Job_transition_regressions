...
transitions and their relationship to poverty/vulnerability outcomes in Brazil.

METHODOLOGY:
1. Takes panel data (2022-2023) with households observed in both periods
2. Creates baseline (t0=2022) and endline (t1=2023) measures for:
   - Socioeconomic status (poverty, vulnerability, income)
   - Labor market status (employment, skill level, sector)
3. Constructs transition variables (fell into poverty, escaped poverty, 
   job entry/exit, skill upgrading/downgrading, same skill)
4. Extends all t0 and t1 values to a single observation per household head
   (keeping only baseline year for cross-sectional regression analysis)
...
OUTPUT:
- One observation per household head with complete t0/t1 transition history
...
global input_path "C:\Users\wb593225\OneDrive - WBG\Desktop\Shared\FY2025\2021PPP\Vulnerability line\Data\Brazil"
global dataset_name "09_BRA_PNAD_Harmonized_2022-2023.dta"
global output_path "C:\Users\wb593225\OneDrive - WBG\Desktop\Shared\FY2025\2021PPP\Vulnerability line\Output\BRA\Regressions Brief"
...
* Set period parameters for single period analysis
local period_t0 2022
local period_t1 2023
local period_desc "2022-2023"

noi di ""
noi di "=== LABOR TRANSITIONS ANALYSIS: BRAZIL 2022-2023 ==="
...
* Step 1: Keep only household heads and identify those present in transition period
keep if jefe == 1
sort idp_i ano
...
* Keep households present in the transition period
keep if in_transition == 1
...
**# ==============================================================================
**# 4. FUNCTION TO CREATE TRANSITION VARIABLES
**# ==============================================================================
...
program define create_transition_vars
    ...
    * Create poverty status for both periods
    gen poor_`t0' = (ipcf_ppp21 <= $poverty_line_monthly_2021ppp) if ano == `t0' & !missing(ipcf_ppp21)
    gen poor_`t1' = (ipcf_ppp21 <= $poverty_line_monthly_2021ppp) if ano == `t1' & !missing(ipcf_ppp21)
    
    * Create vulnerability status for both periods
    gen vuln_`t0' = (ipcf_ppp21 <= $vul_line_monthly) if ano == `t0' & !missing(ipcf_ppp21)
    gen vuln_`t1' = (ipcf_ppp21 <= $vul_line_monthly) if ano == `t1' & !missing(ipcf_ppp21)
    
    * NEW: Create income level variables for both periods
    gen ipcf_ppp21_`t0' = ipcf_ppp21 if ano == `t0'
    gen ipcf_ppp21_`t1' = ipcf_ppp21 if ano == `t1'
    
    * Extend poverty and vulnerability status to both years for each individual
    sort idp_i ano
    by idp_i: egen poor_t0 = max(poor_`t0')
    by idp_i: egen poor_t1 = max(poor_`t1')
    by idp_i: egen vuln_t0 = max(vuln_`t0')
    by idp_i: egen vuln_t1 = max(vuln_`t1')
    
    * NEW: Extend income variables to both years for each individual
    by idp_i: egen ipcf_ppp21_t0 = max(ipcf_ppp21_`t0')
    by idp_i: egen ipcf_ppp21_t1 = max(ipcf_ppp21_`t1')
    
    * Create other time-varying variables for t0 and t1
    gen employed_`t0' = employed if ano == `t0'
    gen employed_`t1' = employed if ano == `t1'
    gen skill_level_`t0' = skill_level if ano == `t0'
    gen skill_level_`t1' = skill_level if ano == `t1'
    gen sector_`t0' = temp_sector if ano == `t0'
    gen sector_`t1' = temp_sector if ano == `t1'
    gen asistencia_`t0' = asistencia if ano == `t0'
    gen asistencia_`t1' = asistencia if ano == `t1'
    
    * Extend to all years for each individual
    by idp_i: egen employed_t0 = max(employed_`t0')
    by idp_i: egen employed_t1 = max(employed_`t1')
    by idp_i: egen skill_level_t0 = max(skill_level_`t0')
    by idp_i: egen skill_level_t1 = max(skill_level_`t1')
    by idp_i: egen sector_t0 = max(sector_`t0')
    by idp_i: egen sector_t1 = max(sector_`t1')
    by idp_i: egen asistencia_t0 = max(asistencia_`t0')
    by idp_i: egen asistencia_t1 = max(asistencia_`t1')
    
    * Create poverty transition outcomes (only falling and escaping)
    gen fell_into_poverty = (poor_t0 == 0 & poor_t1 == 1)
    gen escaped_poverty = (poor_t0 == 1 & poor_t1 == 0)
    
    * Create vulnerability transition outcomes (only falling and escaping)
    gen fell_into_vulnerability = (vuln_t0 == 0 & vuln_t1 == 1)
    gen escaped_vulnerability = (vuln_t0 == 1 & vuln_t1 == 0)
    
    * Employment transition variables
    gen entered_job = (employed_t0 == 0 & employed_t1 == 1)
    gen exited_job = (employed_t0 == 1 & employed_t1 == 0)
    
    * Job-to-job transitions (for those employed both periods)
    gen skill_increased = (employed_t0==1 & employed_t1==1 & skill_level_t1 > skill_level_t0)
    gen skill_decreased = (employed_t0==1 & employed_t1==1 & skill_level_t1 < skill_level_t0)
    
    * NEW: Same skill transition (remained employed with same skill level)
    gen same_skill = (employed_t0==1 & employed_t1==1 & skill_level_t0 == skill_level_t1 & !missing(skill_level_t0) & !missing(skill_level_t1))
    
    * Create skill level dummies for t0
    gen skill_level_1_t0 = (skill_level_t0 == 1)
    gen skill_level_2_t0 = (skill_level_t0 == 2)
    gen skill_level_3_t0 = (skill_level_t0 == 3)
    
    * Transfer changes (for dataset saving)
    gen stopped_transfers = (asistencia_t0 == 1 & asistencia_t1 == 0) if !missing(asistencia_t0) & !missing(asistencia_t1)
    gen started_transfers = (asistencia_t0 == 0 & asistencia_t1 == 1) if !missing(asistencia_t0) & !missing(asistencia_t1)
    replace stopped_transfers = 0 if missing(stopped_transfers)
    replace started_transfers = 0 if missing(started_transfers)
...
program define create_control_vars
    ...
    * Region clustering variable from t0
    ...
    bysort idp_h idp_i: egen region_est1_t0 = max(region_est1_num * (ano == `t0'))
    
    * Household size and children (measured at t0)
    bysort idp_h: egen hh_members_t0 = max(miembros * (ano == `t0')) if !missing(miembros)
    bysort idp_h: egen hh_children_t0 = max(nro_hijos * (ano == `t0')) if !missing(nro_hijos)
    
    * Partner status controls
    * Partner status at t0
    bysort idp_h idp_i: egen partner_t0 = max(has_partner_`t0')
    
    * Household head education level at t0
    bysort idp_h idp_t: egen hh_head_education = max(nivedu * jefe * (ano == `t0')) if !missing(nivedu) & !missing(jefe)
    
    * Health shock (for dataset saving, but won't use in regression)
    bysort idp_h: egen hh_head_health_shock = max(enfermo * (ano == `t0')) if !missing(enfermo)
    
    * Age group variables based on gedad from t0 (baseline: gedad==2)
    bysort idp_h idp_i: egen gedad_t0 = max(gedad * (ano == `t0'))
    gen gedad_25_40 = (gedad_t0 == 3) if !missing(gedad_t0)
    gen gedad_41_64 = (gedad_t0 == 4) if !missing(gedad_t0) 
    gen gedad_65plus = (gedad_t0 == 5) if !missing(gedad_t0)
    
    * Urban status at t0
    bysort idp_h idp_i: egen urbano_t0 = max(urbano * (ano == `t0'))
    
    * Gender (time-invariant but capture at t0)
    bysort idp_h idp_i: egen hombre_t0 = max(hombre * (ano == `t0'))
    
    * Working household members control (equals 1 if head employed at t0)
    gen n_workers_t0 = employed_t0
...
* Create transition variables for the single period (2022-2023)
...
save "$save_path/02_BRA_reg_data_2022-2023.dta", replace

noi di "Dataset saved: $save_path/02_BRA_reg_data_2022-2023.dta"

**# ==============================================================================
**# 8. DEFINE CONTROL SETS
**# ==============================================================================

* Modified control set (same as Peru - removed marital status, health shock, sector, transfers)
...
**# ==============================================================================
**# 9. REGRESSION ANALYSIS 1: FALLING INTO POVERTY
**# ==============================================================================
...
**# ==============================================================================
**# 10. REGRESSION ANALYSIS 2: ESCAPING POVERTY
**# ==============================================================================
...
**# ==============================================================================
**# 11. REGRESSION ANALYSIS 3: FALLING INTO VULNERABILITY
**# ==============================================================================
...
**# ==============================================================================
**# 12. REGRESSION ANALYSIS 4: ESCAPING VULNERABILITY
**# ==============================================================================
...
if _rc == 0 {
    ...
    outreg2 using "$output_path/01_falling_poverty_BRA.xls", ///
        replace excel label title("Falling into Poverty - Brazil") ///
...
if _rc == 0 {
    ...
    outreg2 using "$output_path/02_escaping_poverty_BRA.xls", ///
        replace excel label title("Escaping Poverty - Brazil") ///
        addstat(Adjusted R-squared, e(r2_a)) addtext(Regional FE, Yes) ///
        ctitle("With Region FE")
...
if _rc == 0 {
    ...
    outreg2 using "$output_path/03_falling_vulnerability_BRA.xls", ///
        replace excel label title("Falling into Vulnerability - Brazil") ///
        addstat(Adjusted R-squared, e(r2_a)) addtext(Regional FE, Yes) ///
        ctitle("With Region FE")
...
if _rc == 0 {
    ...
    outreg2 using "$output_path/04_escaping_vulnerability_BRA.xls", ///
        replace excel label title("Escaping Vulnerability - Brazil") ///
        addstat(Adjusted R-squared, e(r2_a)) addtext(Regional FE, Yes) ///
        ctitle("With Region FE")
...
noi di "  - Same modified control set as Peru"
noi di "  - Vulnerability analysis (17.0 USD/day threshold)"
noi di "  - Two-column format: With/Without region FE"
noi di ""
noi di "NEW VARIABLES ADDED:"
noi di "  - ipcf_ppp21_t0: Per capita income at baseline (2022)"
noi di "  - ipcf_ppp21_t1: Per capita income at endline (2023)"
...
noi di "  01_falling_poverty_BRA.xls"
noi di "  02_escaping_poverty_BRA.xls" 
noi di "  03_falling_vulnerability_BRA.xls"
noi di "  04_escaping_vulnerability_BRA.xls"
noi di ""
noi di "DATASET SAVED:"
noi di "  02_BRA_reg_data_2022-2023.dta"
noi di ""
